...
filename = sys.argv[1]
# O segundo argumento é o tipo de exercício
exercise_type = sys.argv[2] if len(sys.argv) > 2 else "sustentacao_vogal" # Padrão para sustentação
...
try:
    ...
    # Prepara os dados do contorno de afinação, que são úteis para ambos os exercícios
    ...
    if exercise_type == "escala_5_notas":
        # Para uma escala, focamos na afinação e na extensão
        ...
        results["summary"] = {
            ...
            "pitch_note": f"Extensão de {frequency_to_note(min_pitch_hz)} a {frequency_to_note(max_pitch_hz)}",
            "intensity_db": sound.get_intensity(),
            "hnr_db": call(sound.to_harmonicity(), "Get mean", 0, 0),
            "duration_seconds": sound.get_total_duration()
        }
        results["vibrato"] = {"is_present": False} # Vibrato não é relevante para escalas
        results["status"] = "Análise de Escala completa."

    else: # O padrão é a análise de sustentação de vogal
        mean_pitch_hz = call(pitch, "Get mean", 0, 0, "Hertz")
        stdev_pitch_hz = call(pitch, "Get standard deviation", 0, 0, "Hertz")
